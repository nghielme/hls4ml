...
# TODO - Paramaterize for Catapult (different strategies?)
@pytest.fixture
...
def convert(load_jettagging_model, strategy):
  ...
  hls4ml.model.optimizer.get_optimizer('output_rounding_saturation_mode').configure(layers=['Activation'], rounding_mode='AC_RND', saturation_mode='AC_SAT')

  config = hls4ml.utils.config_from_keras_model(model, granularity='name')
  config['Model']['Strategy'] = strategy
  config['LayerName']['softmax']['exp_table_t'] = 'ac_fixed<18,8,true>'
  config['LayerName']['softmax']['inv_table_t'] = 'ac_fixed<18,4,true>'
  config['LayerName']['softmax']['Precision'] = 'ac_fixed<16,6,false>'
...
def test_single_dense_activation_exact(randX_100_16, bits, alpha, backend):
  ...
  hls4ml.model.optimizer.get_optimizer('output_rounding_saturation_mode').configure(layers=['relu1'], rounding_mode='AC_RND_CONV', saturation_mode='AC_SAT')
  config = hls4ml.utils.config_from_keras_model(model, granularity='name')
...
def test_quantizer(randX_1000_1, quantizer, backend):
  ...
  '''
  X = randX_1000_1
  X = np.round(X * 2**10) * 2**-10 # make it an exact ac_fixed<16,6,true>
  model = Sequential()
  model.add(QActivation(input_shape=(1,), activation=quantizer, name='quantizer'))
  model.compile()

  hls4ml.model.optimizer.get_optimizer('output_rounding_saturation_mode').configure(layers=['quantizer'], rounding_mode='AC_RND_CONV', saturation_mode='AC_SAT')
  config = hls4ml.utils.config_from_keras_model(model, granularity='name')
...
def test_qactivation_kwarg(randX_100_10,
    ...
    hls4ml.model.optimizer.get_optimizer(
        ...
        layers=[name],
        rounding_mode='AC_RND_CONV',
        saturation_mode='AC_SAT'
    )
