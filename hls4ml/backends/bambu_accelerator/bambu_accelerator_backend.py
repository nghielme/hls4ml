...
def _build_manifest(project_dir: str, project_name: str,
    ...
    from hls4ml.backends.bambu_accelerator.wrapper import (
        parse_module, build_rename_map, extract_data_widths, extract_bram_depths,
    )
...
def _patch_params(project_dir: str, flow: str, data_widths: dict,
                  n_words: dict, bram_slots: dict | None) -> None:
    ...
    def build(
        ...
        *,
        ...
        if synth:
            ...
            _patch_params(project_dir, flow, manifest['data_widths'],
                          manifest['n_words'], manifest['bram_slots'])
