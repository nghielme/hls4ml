...
def parse_module(content: str) -> tuple[str, list[str], PortDecls]:
    ...
    port_decls: PortDecls = {
        m.group(3): (m.group(1), (m.group(2) or '').strip())
...
def _build_rename_map_axis(port_names: list[str],
                           port_decls: PortDecls) -> dict[str, str]:
...
def build_rename_map(port_names: list[str],
                     port_decls: PortDecls,
                     flow: str) -> dict[str, str]:
...
def generate_wrapper_verilog(module_name: str, port_names: list[str],
                             port_decls: PortDecls, flow: str) -> str:
...
def extract_data_widths(port_names: list[str], port_decls: PortDecls,
    ...
    """
    if flow == 'stream':
        in_tdatas  = [p for p in port_names
                      if p.endswith('_TDATA') and port_decls.get(p, ('', ''))[0] == 'input']
        out_tdatas = [p for p in port_names
                      if p.endswith('_TDATA') and port_decls.get(p, ('', ''))[0] == 'output']
