...
def solve_pll(input_freq_mhz: float, targets: list[float], use_external_oscillator: bool = False, hdl: str = 'verilog') -> str:
    """
    ...
    dyn_ratios = {2:0, 4:1, 6:2, 8:3, 10:4, 20:5, 40:6, 60:7, 
                  80:8, 100:9, 200:10, 400:11, 600:12, 800:13, 1000:14, 2000:15}
    ...
    for t in targets:
        ...
        if not feasible:
            raise Exception(f"Hardware cannot generate {t} MHz. Check that the target frequency has correct precision. We check for tol < 1e-5, i.e. 33.33332 < 100.0/3.0 < 33.33334.")
    ...
    for t_idx in range(len(targets)):
        model.Add(sum(assign[t_idx, p, v] 
                      for p in range(MAX_PLLS) 
                      for v in range(len(vco_list))) == 1)
    ...
    for p in range(MAX_PLLS):
        for v_idx in range(len(vco_list)):
            ...
            for s_idx in range(4):
                stat_vars = [use_stat[t, p, v_idx, s_idx] for t in range(len(targets)) 
                             if (t, p, v_idx, s_idx) in use_stat]
    ...
    if status in (cp_model.OPTIMAL, cp_model.FEASIBLE):
        ...
        if hdl == 'vhdl':
            return generate_vhdl_ortools(solver, x, assign, use_stat, use_dyn, targets, vco_list, candidate_vcos, dyn_ratios, static_maps, use_external_oscillator)
        else:
            return generate_verilog_ortools(solver, x, assign, use_stat, use_dyn, targets, vco_list, candidate_vcos, dyn_ratios, static_maps, use_external_oscillator)
    else:
        raise Exception("No valid configuration found.")

def generate_vhdl_ortools(solver, x, assign, use_stat, use_dyn, targets, vco_list, vco_configs, dyn_ratios, static_maps, use_external_oscillator=False):
    """
    ...
    Args:
        ...
        static_maps (list[dict]): List of dictionaries mapping static divider ratios to configuration codes for each static port.
        use_external_oscillator (bool): Whether to use an external oscillator (default: False).
    ...
    for p in range(MAX_PLLS):
        ...
        if active_v_idx == -1: continue # PLL unused
        ...
        g = {}
        # Init defaults
        for i in range(1,5): g[f"clk_outdiv{i}"] = (0, 3)
        for i in range(1,6): g[f"clk_outdivd{i}"] = (0, 4)
        ports_map = {}
        for i in range(1,5): ports_map[f"CLK_DIV{i}"] = "open"
        ...
        for t_idx, t in enumerate(targets):
            ...
            if solver.Value(assign[t_idx, p, active_v_idx]) == 0: continue
...
def generate_verilog_ortools(solver, x, assign, use_stat, use_dyn, targets, vco_list, vco_configs, dyn_ratios, static_maps, use_external_oscillator=False):
    """
    ...
    for p in range(MAX_PLLS):
        ...
        for i in range(1, 6): g[f"clk_outdivd{i}"] = (0, 4)
        ports_map = {}
        for i in range(1, 5): ports_map[f"CLK_DIV{i}"] = "()"
        for i in range(1, 6): ports_map[f"CLK_DIVD{i}"] = "()"
